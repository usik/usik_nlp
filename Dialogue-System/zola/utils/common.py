...
def read_global_config() -> Dict[Text, Any]:
    ...
    try:
        return zola.utils.io.read_config_file(GLOBAL_USER_CONFIG_PATH)
    except Exception:
...
def set_log_level(log_level: Optional[int] = None):
    """Set log level of Zola and Tensorflow either to the provided log level or
    ...
    logging.getLogger("zola").setLevel(log_level)
...
def write_global_config_value(name: Text, value: Any) -> None:
    ...
    try:
        ...
        c[name] = value
        zola.dm.utils.dump_obj_as_yaml_to_file(GLOBAL_USER_CONFIG_PATH, c)
...
def read_global_config_value(name: Text, unavailable_ok: bool = True) -> Any:
    """Read a value from the global Bot configuration."""
...
def mark_as_experimental_feature(feature_name: Text) -> None:
    ...
    logger.warning(
        "{} 이부분은 아직 실험용입니다. "
        "피드백에 따라서 유지할지 지울지 결정하겠습니다.".format(feature_name)
    )
